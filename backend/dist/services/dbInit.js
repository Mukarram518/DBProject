...
async function seedInitialData(connection) {
    ...
    if (usersCount[0].count === 0) {
        ...
        for (const u of initialUsers) {
            await connection.query("INSERT INTO users (first_name, last_name, email, password, phone, role, status) VALUES (?, ?, ?, ?, ?, ?, ?)", u);
        }
    }
    ...
    if (prodCount[0].count === 0) {
        ...
        const [cats] = await connection.query("SELECT category_id, category_name FROM categories");
        const catMap = new Map();
        for (const cat of cats)
            catMap.set(cat.category_name, cat.category_id);
        const products = [
        ...
        for (const p of products) {
            await connection.query("INSERT INTO products (product_name, category_id, price, stock_quantity, description) VALUES (?, ?, ?, ?, ?)", p);
        }
    }
    ...
    if (orderCount[0].count === 0) {
        ...
        const [uRows] = await connection.query("SELECT user_id FROM users LIMIT 3");
        const [pRows] = await connection.query("SELECT product_id, price FROM products LIMIT 3");
        if (uRows.length > 0 && pRows.length > 0) {
            const uId = uRows[0].user_id;
            const pId = pRows[0].product_id;
            const pPrice = Number(pRows[0].price);
            const [ordRes] = await connection.query("INSERT INTO orders (user_id, total_amount, order_status) VALUES (?, ?, ?)", [uId, pPrice * 2, "Delivered"]);
            const orderId = ordRes.insertId;
            await connection.query("INSERT INTO order_items (order_id, product_id, quantity, price) VALUES (?, ?, ?, ?)", [orderId, pId, 2, pPrice]);
            await connection.query("INSERT INTO payments (order_id, amount, payment_method, payment_status) VALUES (?, ?, ?, ?)", [orderId, pPrice * 2, "Credit Card", "Completed"]);
        }
    }
}
