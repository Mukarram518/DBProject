...
export async function getUserById(req: Request, res: Response) {
  try {
    ...
    const [rows]: any = await pool.query(
      "SELECT user_id, first_name, last_name, email, phone, role, status, created_at FROM users WHERE user_id = ? LIMIT 1",
      [id]
    );
...
export async function createUser(req: Request, res: Response) {
  try {
    ...
    const [existing]: any = await pool.query("SELECT user_id FROM users WHERE email = ? LIMIT 1", [email]);
    ...
    const [result]: any = await pool.query(
      ...
      [firstName, lastName, email, hashedPassword, phone || null, dbRole === "Admin" ? "Admin" : "Customer", status]
    );
...
export async function updateUser(req: Request, res: Response) {
  try {
    ...
    const [rows]: any = await pool.query("SELECT * FROM users WHERE user_id = ? LIMIT 1", [id]);
    if (!rows || rows.length === 0) {
      return res.status(404).json({ success: false, message: "User not found." });
    }
    ...
    await pool.query(
      "UPDATE users SET first_name = ?, last_name = ?, email = ?, phone = ?, role = ?, status = ? WHERE user_id = ?",
      [firstName, lastName, updatedEmail, updatedPhone, updatedRole === "Admin" ? "Admin" : "Customer", updatedStatus, id]
    );
...
export async function deleteUser(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const [result]: any = await pool.query("DELETE FROM users WHERE user_id = ?", [id]);
